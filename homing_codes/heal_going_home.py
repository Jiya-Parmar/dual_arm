...
for the HEAL arm from its current configuration to a zero "home" pose.

- Subscribes:  /joint_states   (sensor_msgs/JointState)
- Publishes:   /velocity_controller/commands (Float64MultiArray)
...
class HealGoHome(Node):
    """
    Simple node:
      1. Waits for one JointState from /joint_states
      2. Builds a QuinticJointSpacePlanner from q_start -> q_home (all zeros)
      3. On each timer step, evaluates q(t), qdot(t) and publishes qdot
      4. Stops (publishes zero) once t >= T
    """
    ...
    def __init__(
        ...
        *,
        ...
        command_topic: str = "/velocity_controller/commands",
        joint_names: Optional[List[str]] = None,
        total_time: float = 10.0,
        rate_hz: float = 200.0,
    ):
        super().__init__("heal_go_home")

        # HEAL joint name order we will use for planning/commands
        ...
        self.q_home = np.zeros(self.n_joints, dtype=float)
        ...
        # QoS matching typical /joint_states (SensorDataQoS)
    ...
    def _extract_q_in_order(self, msg: JointState) -> np.ndarray:
        """Return joint vector q in self.joint_names order."""
    ...
    def _maybe_init_planner(self, now_sec: float):
        ...
        self.get_logger().info(
            "Initialized QuinticJointSpacePlanner for HEAL.\n"
    ...
    def _on_timer(self):
        ...
        if t >= self.total_time:
            ...
            self._done = True
            self.get_logger().info(
                f"[HEAL] Reached end of trajectory (t={t:.3f} >= T={self.total_time:.3f}). "
        ...
        # Optional clamp to safe velocity
        vmax = 0.5  # rad/s, tune as you like
...
def main(args=None):
    ...
    node = HealGoHome(
        ...
        total_time=10.0,   # e.g., 10 sec to go home
        rate_hz=200.0,
    )
